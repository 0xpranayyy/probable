      ...
      <div style={{ maxWidth: 1180, margin: "0 auto", padding: "48px 32px 96px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
          <div>
            ...
            <p style={{ margin: 0, color: "#6E6787", fontSize: 15 }}>
          ...
          <button onClick={() => "Notification" in window && Notification.requestPermission()}
            style={{ background: "#fff", border: "1px solid rgba(29,24,50,.14)", color: "#1D1832", font: "600 12.5px 'Instrument Sans'", padding: "9px 16px", borderRadius: 999, cursor: "pointer" }}>
            Enable notifications
          </button>
        </div>

        {loading ? (
          <div style={{ color: "#A9A2BE", font: "500 13px 'JetBrains Mono'", padding: "40px 0" }}>Loading…</div>
        ) : !items.length ? (
          <div style={{ padding: "48px 0", color: "#6E6787", fontSize: 14.5 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10, marginTop: 22 }}>
            {items.map((it) => {
              ...
              return (
                <div key={it.eventId} style={{ display: "flex", alignItems: "center", gap: 14, background: "#fff", border: "1px solid rgba(29,24,50,.08)", borderRadius: 14, padding: "14px 18px" }}>
                  {it.image && <img src={it.image} alt="" width={38} height={38} style={{ borderRadius: 9, objectFit: "cover", flex: "none" }} />}
                  <Link href={`/market/${it.slug}`} style={{ flex: 1, minWidth: 0, textDecoration: "none", color: "#1D1832" }}>
                    <div style={{ fontWeight: 700, fontSize: 14.5, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{it.title}</div>
                    <div style={{ font: "500 11.5px 'JetBrains Mono'", color: "#A9A2BE", marginTop: 3 }}>
