...
const docSections = [
  ...
  },
  {
    ...
    params: [
      { name: "marketId", type: "string", typeColor: "#8200FF", desc: "Target prediction market ID." },
      { name: "userId", type: "string", typeColor: "#8200FF", desc: "Abstract user ID executing the trade." },
    ...
    <div style={{ minHeight: "100vh", background: "#F8F8FA", color: "#120F24", fontFamily: "'Instrument Sans',sans-serif" }}>
      ...
      <div data-screen-label="Docs" className="sidebar-layout" style={{ maxWidth: "1180px", margin: "0 auto", padding: "48px 32px 96px", alignItems: "start" }}>
        ...
        <div style={{ position: "sticky", top: "98px", display: "flex", flexDirection: "column", gap: "3px" }}>
          <div style={{ font: "600 10.5px 'JetBrains Mono',monospace", color: "#9490A8", letterSpacing: "1.4px", padding: "14px 14px 10px" }}>AUTHENTICATION</div>
          <button 
            onClick={() => setActiveSectionId("keys")}
            style={{ textAlign: "left", background: activeSectionId === "keys" ? "rgba(130,0,255,.06)" : "transparent", border: "none", color: "#120F24", font: "600 14px 'Instrument Sans',sans-serif", padding: "9px 14px", borderRadius: "10px", cursor: "pointer", transition: "background .15s" }}>
            Generate Keys
          </button>

          <div style={{ font: "600 10.5px 'JetBrains Mono',monospace", color: "#9490A8", letterSpacing: "1.4px", padding: "14px 14px 10px" }}>CORE PRIMITIVES</div>
          <button 
            onClick={() => setActiveSectionId("markets")}
            style={{ textAlign: "left", background: activeSectionId === "markets" ? "rgba(130,0,255,.06)" : "transparent", border: "none", color: "#120F24", font: "600 14px 'Instrument Sans',sans-serif", padding: "9px 14px", borderRadius: "10px", cursor: "pointer", transition: "background .15s" }}>
            Create Market
          </button>
          <button 
            onClick={() => setActiveSectionId("wallets")}
            style={{ textAlign: "left", background: activeSectionId === "wallets" ? "rgba(130,0,255,.06)" : "transparent", border: "none", color: "#120F24", font: "600 14px 'Instrument Sans',sans-serif", padding: "9px 14px", borderRadius: "10px", cursor: "pointer", transition: "background .15s" }}>
            Abstract Wallet
          </button>
          <button 
            onClick={() => setActiveSectionId("trades")}
            style={{ textAlign: "left", background: activeSectionId === "trades" ? "rgba(130,0,255,.06)" : "transparent", border: "none", color: "#120F24", font: "600 14px 'Instrument Sans',sans-serif", padding: "9px 14px", borderRadius: "10px", cursor: "pointer", transition: "background .15s" }}>
            Execute Trade
          </button>

          <div style={{ font: "600 10.5px 'JetBrains Mono',monospace", color: "#9490A8", letterSpacing: "1.4px", padding: "14px 14px 10px" }}>ANALYTICS & AI</div>
          <button 
            onClick={() => setActiveSectionId("predictions")}
            style={{ textAlign: "left", background: activeSectionId === "predictions" ? "rgba(130,0,255,.06)" : "transparent", border: "none", color: "#120F24", font: "600 14px 'Instrument Sans',sans-serif", padding: "9px 14px", borderRadius: "10px", cursor: "pointer", transition: "background .15s" }}>
            AI Forecast
          </button>
          ...
          <div style={{ marginTop: "22px", background: "linear-gradient(115deg,#FFE3EC,#EFE9FF)", borderRadius: "14px", padding: "18px" }}>
            ...
            <Link href="/onboard" style={{ display: "inline-block", background: "#120F24", border: "none", color: "#fff", font: "600 12.5px 'Instrument Sans',sans-serif", padding: "8px 15px", borderRadius: "999px", cursor: "pointer", textDecoration: "none" }}>Get keys →</Link>
          </div>
        </div>
        ...
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "8px" }}>
            ...
            <span style={{ font: "600 15px 'JetBrains Mono',monospace", color: "#625E77" }}>{activeSection.path}</span>
          </div>
          
          <h1 style={{ margin: "0 0 14px", font: "800 36px/1.06 'Bricolage Grotesque',sans-serif", letterSpacing: "-1.4px" }}>{activeSection.title}</h1>
          <p style={{ color: "#625E77", fontSize: "16px", lineHeight: "1.65", maxWidth: "560px", margin: "0 0 34px" }}>{activeSection.desc}</p>
          
          <div style={{ font: "600 12px 'JetBrains Mono',monospace", color: "#9490A8", letterSpacing: "1.2px", marginBottom: "14px" }}>PARAMETERS</div>
          <div style={{ border: "1px solid rgba(130,0,255,.09)", borderRadius: "14px", overflow: "hidden", marginBottom: "36px", background: "#fff" }}>
            {activeSection.params.map((pr, j) => (
              <div key={j} style={{ display: "grid", gridTemplateColumns: "190px 110px 1fr", gap: "16px", padding: "15px 20px", borderBottom: "1px solid rgba(130,0,255,.06)", fontSize: "14px", alignItems: "baseline" }}>
                <span style={{ font: "600 13px 'JetBrains Mono',monospace", color: "#120F24" }}>{pr.name}</span>
                <span style={{ font: "500 11.5px 'JetBrains Mono',monospace", color: pr.typeColor }}>{pr.type}</span>
                <span style={{ color: "#625E77", lineHeight: "1.55" }}>{pr.desc}</span>
              </div>
            ))}
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1.1fr .9fr", gap: "24px", marginBottom: "36px" }}>
            <div>
              <div style={{ font: "600 12px 'JetBrains Mono',monospace", color: "#9490A8", letterSpacing: "1.2px", marginBottom: "14px" }}>EXAMPLE REQUEST</div>
              <div style={{ background: "#120F24", borderRadius: "15px", padding: "22px", font: "500 12.5px/1.8 'JetBrains Mono',monospace", color: "#B3A8D9", whiteSpace: "pre-wrap", overflowX: "auto" }}>
                {activeSection.example}
              </div>
            </div>

            {/* Sandbox Try It Out Console */}
            <div style={{ background: "#fff", border: "1px solid rgba(130,0,255,.08)", borderRadius: "16px", padding: "20px" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                {activeSection.id !== "keys" && (
                  <div>
                    ...
                    <input 
                      ...
                      style={{ width: "100%", background: "#F8F8FA", border: "1px solid rgba(130,0,255,.12)", borderRadius: "8px", padding: "8px 10px", font: "500 12px 'JetBrains Mono',monospace" }}
                    />
                  </div>
                )}
                ...
                <button 
                  ...
                  style={{ background: "#120F24", border: "none", color: "#fff", font: "700 13px 'Instrument Sans',sans-serif", padding: "10px", borderRadius: "8px", cursor: "pointer", marginTop: "8px" }}>
          ...
          {sandboxResponse && (
            <div>
              <div style={{ font: "600 12px 'JetBrains Mono',monospace", color: "#9490A8", letterSpacing: "1.2px", marginBottom: "14px" }}>SANDBOX RESPONSE</div>
              <pre style={{ background: "#0E0B1A", border: "1px solid rgba(255,92,35,.3)", borderRadius: "15px", padding: "20px", font: "500 12.5px 'JetBrains Mono',monospace", color: "#17B877", overflowX: "auto" }}>
